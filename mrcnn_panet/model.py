...
class PyramidROIAlign_AFN(KL.Layer):
    ...
    def __init__(self, pool_shape,index, **kwargs):
        super(PyramidROIAlign_AFN, self).__init__(**kwargs)
    ...
    def call(self, inputs):
        ...
        for i, level in enumerate(range(2, 6)):
            ix = tf.compat.v1.where(tf.equal(roi_level2, level))
        ...
        box_to_level3 = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.compat.v1.where(tf.equal(roi_level3, level))
            level_boxes = tf.gather_nd(boxes, ix)
        ...
        box_to_level4 = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.compat.v1.where(tf.equal(roi_level4, level))
            level_boxes = tf.gather_nd(boxes, ix)
        ...
        box_to_level5 = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.compat.v1.where(tf.equal(roi_level5, level))
            level_boxes = tf.gather_nd(boxes, ix)
...
def build_fpn_mask_graph(rois, feature_maps, image_meta,
    ...
    s = K.int_shape(x1)
    x1 = KL.Reshape((-1,28,28, num_classes), name="mrcnn_mask_fc_reshape")(x1)
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            if len(config.EDGE_LOSS_FILTERS) > 0:##对比之下多了这里 FLAG！！
