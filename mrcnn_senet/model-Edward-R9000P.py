...
ctx._force_start_method('spawn')#将fork模式更改为spawn来解决多线程问题
...
def compute_backbone_shapes(config, image_shape):
    ...
    return np.array(
        [[int(math.ceil(image_shape[0] / stride)),
            ...
            for stride in config.BACKBONE_STRIDES])#这里的步长为4，8，16，32，64来构造FPN
...
def Fully_connected(x, units=None,name='fully_connected') :
    with tf.compat.v1.name_scope(name) :
        return tf.keras.layers.Dense(use_bias=False,units=units)(x)
def SENet(x,name="SENet"):
    channel=x.shape.as_list()[-1]
    squeeze=Global_Average_Pooling(x)
    # squeeze=tf.keras.layers.Reshape((1, 1, channel))(squeeze)
    excitation=Fully_connected(squeeze,units=int(channel/16),name=name+"_excitation_1")
    # excitation = KL.Conv2D(filters=channel//16, kernel_size=1, strides=(1,1),name=name+"_excitation_1")(squeeze)
    excitation=KL.Dropout(0.5)(excitation)
    excitation = KL.Activation('relu')(excitation)
    excitation=Fully_connected(excitation,units=channel,name=name+"_excitation_2")
    # excitation = KL.Conv2D(filters=channel,kernel_size=1, strides=(1,1),name=name+"_excitation_2")(excitation)
    excitation = KL.Activation('sigmoid')(excitation)
    excitation = tf.keras.layers.Reshape((1, 1, channel))(excitation)
    return KL.multiply([excitation,x])
...
def identity_block(input_tensor, kernel_size, filters, stage, block,
    ...
    # Arguments
        ...
        stage: integer, current stage label, used for generating la yer names
    ...
    se_name_base = 'se'+str(stage) + block + '_branch'
    ...
    x = KL.Conv2D(nb_filter3, (1, 1), name=conv_name_base + '2c',
                  use_bias=use_bias)(x)
    # x = SENet(x,name=se_name_base)## 新加的SENet结构
...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = SENet(x,name=se_name_base)##加入SeNet
    x = KL.Add()([x, shortcut])
    x = KL.Activation('relu', name='res' + str(stage) + block + '_out')(x)
    return x
...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    # Stage 1
    x = KL.ZeroPadding2D((3, 3))(input_image)# padding top and bottom 3 lines 0
...
def build_rpn_model(anchor_stride, anchors_per_location, depth):#the depth here means the number of channels
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        if mode == "training":
            ...
            input_gt_boxes = KL.Input(
                shape=[None, 4], name="input_gt_boxes", dtype=tf.float32)# 4=num(y+x)
    ...
    def find_last(self):
        ...
        print("选取的checkpoint为"+checkpoint)
        return checkpoint
    ...
    def compile(self, learning_rate, momentum):
        ...
        if len(self.config.EDGE_LOSS_FILTERS) > 0:##这里多加 FLAG！！
